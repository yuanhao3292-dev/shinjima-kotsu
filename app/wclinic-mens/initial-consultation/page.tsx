...
const pageTranslations = {
  serviceName: { ja: 'Wクリニック メンズ 初期相談サービス', 'zh-TW': 'W CLINIC men\'s 前期諮詢服務', 'zh-CN': 'W CLINIC men\'s 前期咨询服务', en: 'W CLINIC men\'s Initial Consultation' } as Record<Language, string>,
  serviceNameEn: { ja: 'W CLINIC men\'s - Initial Consultation', 'zh-TW': 'W CLINIC men\'s - Initial Consultation', 'zh-CN': 'W CLINIC men\'s - Initial Consultation', en: 'W CLINIC men\'s - Initial Consultation' } as Record<Language, string>,
  serviceDescription: { ja: '病歴翻訳・クリニック相談・ED/LOH/AGA治療プラン評価・費用概算', 'zh-TW': '病歷翻譯、診所諮詢、ED/LOH/AGA治療方案評估、費用概算', 'zh-CN': '病历翻译、诊所咨询、ED/LOH/AGA治疗方案评估、费用概算', en: 'Medical record translation, clinic consultation, ED/LOH/AGA treatment assessment, cost estimate' } as Record<Language, string>,
  serviceLongDescription: { ja: 'W CLINIC men\'s向けの初期相談サービス。診療情報を翻訳（中→日）し、植村天受教授による初診評価を行い、ED/LOH/AGA治療の可能性を評価し、費用概算をご提供します。', 'zh-TW': '專為W CLINIC men\'s提供的前期諮詢服務。我們將翻譯您的病歷資料（中→日），由植村天受教授進行初診評估，評估ED/LOH/AGA治療可行性並提供費用概算。', 'zh-CN': '专为W CLINIC men\'s提供的前期咨询服务。我们将翻译您的病历资料（中→日），由植村天受教授进行初诊评估，评估ED/LOH/AGA治疗可行性并提供费用概算。', en: 'Initial consultation for W CLINIC men\'s. We translate your medical records (CN→JP), arrange initial assessment by Prof. Uemura, evaluate ED/LOH/AGA treatment feasibility and provide cost estimates.' } as Record<Language, string>,
  ...
  feature1: { ja: '診療情報の翻訳（中→日）', 'zh-TW': '病歷資料翻譯（中文→日文）', 'zh-CN': '病历资料翻译（中文→日文）', en: 'Medical record translation (CN→JP)' } as Record<Language, string>,
  feature2: { ja: '植村天受教授による初診評価', 'zh-TW': '植村天受教授初診評估', 'zh-CN': '植村天受教授初诊评估', en: 'Initial assessment by Prof. Uemura' } as Record<Language, string>,
  feature3: { ja: 'ED/LOH/AGA治療プランの策定', 'zh-TW': 'ED/LOH/AGA治療方案制定', 'zh-CN': 'ED/LOH/AGA治疗方案制定', en: 'ED/LOH/AGA treatment plan development' } as Record<Language, string>,
  feature4: { ja: '費用概算とスケジュールのご説明', 'zh-TW': '詳細費用概算與日程安排', 'zh-CN': '详细费用概算与日程安排', en: 'Detailed cost estimation and scheduling' } as Record<Language, string>,
  feature5: { ja: '来日受診の行程アドバイス', 'zh-TW': '赴日就診行程建議', 'zh-CN': '赴日就诊行程建议', en: 'Travel advice for visiting Japan' } as Record<Language, string>,
  ...
  doc1: { ja: '既往病歴・診断書', 'zh-TW': '既往病歷/診斷書', 'zh-CN': '既往病历/诊断书', en: 'Medical history / diagnosis report' } as Record<Language, string>,
  doc2: { ja: '血液検査報告書（テストステロン値優先）', 'zh-TW': '血液檢查報告（含睪固酮水平優先）', 'zh-CN': '血液检查报告（含睾酮水平优先）', en: 'Blood test report (testosterone level preferred)' } as Record<Language, string>,
  doc3: { ja: '現在服用中のお薬リスト', 'zh-TW': '目前用藥清單', 'zh-CN': '目前用药清单', en: 'Current medication list' } as Record<Language, string>,
  doc4: { ja: 'これまでの治療経過', 'zh-TW': '既往治療經過', 'zh-CN': '既往治疗经过', en: 'Previous treatment history' } as Record<Language, string>,
  ...
  currentStatus: { ja: '現在の治療状況', 'zh-TW': '目前治療情況', 'zh-CN': '目前治疗情况', en: 'Current Treatment Status' } as Record<Language, string>,
  currentStatusPlaceholder: { ja: '現在の治療状況、治療経過等を簡単にご記入ください', 'zh-TW': '請簡述目前的治療情況、治療經過等', 'zh-CN': '请简述目前的治疗情况、治疗经过等', en: 'Brief description of current treatment and history' } as Record<Language, string>,
...
const SERVICE_INFO = {
  id: MEDICAL_PACKAGES['wclinic-mens-initial-consultation'].slug,
  slug: MEDICAL_PACKAGES['wclinic-mens-initial-consultation'].slug,
  price: MEDICAL_PACKAGES['wclinic-mens-initial-consultation'].priceJpy,
};

export default function WClinicMensInitialConsultationPage() {
  ...
  return (
    <CheckoutLayout>
      ...
      <div className="bg-gradient-to-r from-slate-900 to-gray-900 py-12">
            ...
            <div className="bg-amber-50 rounded-2xl p-6 border border-amber-100 sticky top-8">
              ...
              <div className="mt-6 pt-6 border-t border-amber-200">
                ...
                <ul className="space-y-2 text-sm text-gray-600">
                  {[t('doc1'), t('doc2'), t('doc3'), t('doc4')].map((doc, idx) => (
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="md:col-span-2">
                      ...
                      <textarea value={patientInfo.currentStatus} onChange={(e) => setPatientInfo({ ...patientInfo, currentStatus: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent" rows={3} placeholder={t('currentStatusPlaceholder')} />
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent" rows={4} placeholder={t('additionalNotesPlaceholder')} />
                </div>
                ...
                <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
                  ...
                  <button type="submit" disabled={processing} className="w-full py-4 bg-gradient-to-r from-slate-900 to-gray-900 text-white font-bold rounded-xl hover:from-slate-800 hover:to-gray-800 transition shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
