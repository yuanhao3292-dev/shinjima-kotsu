...
const FAQ_DATA: FAQItem[] = [
  ...
  },
  {
    category: 'cancer',
    question: {
      ja: '渡日治療の費用はどのくらいかかりますか？',
      'zh-TW': '去日本治療癌症大概需要花多少錢？',
      'zh-CN': '去日本治疗癌症大概需要花多少钱？',
      en: 'How much does cancer treatment in Japan cost?',
    },
    answer: {
      ja: '費用は病種・治療法により大きく異なります。参考価格：（1）陽子線治療：約250-350万円（2）消化器系がん手術：約300-500万円（3）総合がん治療（手術＋化学療法＋放射線）：約1,500-2,000万円。医療費以外に、仲介サービス料、医療通訳費、渡航費・宿泊費なども必要です。外国人患者は日本の国民健康保険を利用できないため全額自費となります。当社では初回相談時に詳細な費用見積もりをお出しし、治療費の事前送金手続きもサポートいたします。',
  ...
  },
  {
    ...
    answer: {
      ja: '言語サポートは万全です：（1）医学知識を持つ中日バイリンガル医療通訳が問診・検査・治療説明の全過程に同席（2）一部の大病院（がん研有明病院、国立国際医療研究センター等）には中国語コーディネーターが常駐（3）診療記録は事前に専門医療翻訳者が日本語に翻訳し、医学的な正確性を確保（4）当社が全過程の通訳手配と入退院時のサポートを行います。一般的な観光通訳では医療場面の正確なコミュニケーションは困難なため、医療通訳資格を持つ専門スタッフの利用を強くお勧めします。',
  ...
  // ===== Health Checkup =====
  {
    category: 'checkup',
    question: {
      ja: '日本の人間ドックには何が含まれますか？中国の体検と何が違いますか？',
  ...
  },
  {
    category: 'checkup',
    question: {
      ja: '体検の予約はどのくらい前に必要ですか？当日の注意事項は？',
    ...
    answer: {
      ja: '国際予約・ビザ手続き・問診票準備等を含め、4-6週間前のご予約をお勧めします。春秋シーズンはさらに早めのご予約が安心です。当日の注意：（1）前日21時以降絶食、当朝は絶食絶飲（少量の水はOK）。PET-CT検査は6時間以上の絶食が必須（2）前日の激しい運動・飲酒は避ける（3）脱着しやすいゆったりした服装で、金属アクセサリーは外す（4）既往の検査結果があればお持ちください。検査時間は半日〜1日、結果は約3-4週間後に中国語レポートを国際便でお届けします。',
  ...
  },
  {
    ...
    answer: {
      ja: '日本のゴルフ場はマナーに厳格です。服装：（1）襟付き袖付きのシャツを着用し、裾はパンツにイン（2）ジーンズ・サンダル・スニーカー・ノースリーブ禁止（3）ソフトスパイクまたはスパイクレスの専用シューズ（金属スパイク禁止）（4）名門クラブでは入退館時にジャケット着用が必要。マナー：（1）スタート時間の30-60分前に到着（2）各ホール15分以内のプレーペース維持（3）グリーンのディボット修復・バンカー均し必須（4）携帯電話はマナーモード/電源OFF（5）コース内での大声禁止。日本特有の慣習として、前半9ホール終了後に約1時間の昼食休憩があります。',
  ...
  },
  {
    ...
    answer: {
      ja: 'ベストシーズンは春（4-5月、桜）と秋（10-11月、紅葉）。夏は北海道が人気、冬は沖縄が最適。費用目安：平日パブリックコース約8,000-15,000円/人、週末約12,000-25,000円/人、名門コース約30,000-80,000円/人。キャディ：中高級コースでは予約時にリクエスト可能（約10,000-15,000円/組・4名共有）。日本のキャディはコース熟知度が非常に高く、距離やライン読みのアドバイスも的確です。なお、日本にはチップの習慣はなく、キャディ料金は総額に含まれます。別途、昼食代、ロッカー代、入湯税等の少額費用がかかります。',
  ...
  },
  {
    ...
    answer: {
      ja: '車種・時間別の参考料金：（1）5人乗りセダン（トヨタクラウン等）：約25,000-30,000円/10時間（2）7人乗りMPV（トヨタアルファード）：約35,000-45,000円/10時間（3）9人乗りワゴン：約40,000-50,000円/10時間（4）大型バス（20-45人乗り）：約80,000-150,000円/日。上記は燃料費・ドライバー料込み、高速料金・駐車場代は実費別途。超過料金は約3,000-5,000円/時間。タクシー（成田→東京市内で約2-3万円）と比較しても、チャーター車はコストパフォーマンスが高く、中国語対応・車内ゆったり・行程自由度の点でお勧めです。',
  ...
  },
  {
    ...
    answer: {
      ja: '予約方法：2-3日前までに当社ウェブサイト・WeChat・電話でご予約ください。便名・到着日時・人数・お名前・目的地をお知らせいただきます。車種と料金確認後、前日にドライバー情報（氏名・電話番号・車両ナンバー）をお送りします。到着ロビーでネームボードを持ったドライバーがお待ちしています。飛行機遅延の場合：当社はフライト状況をリアルタイムで監視しており、遅延時はドライバーが自動的に待機時間を調整します。2時間以内の遅延は無料で対応、2時間超過は超過料金（約3,000-5,000円/時間）が発生します。欠航の場合は無料で日程変更または全額返金いたします。',
