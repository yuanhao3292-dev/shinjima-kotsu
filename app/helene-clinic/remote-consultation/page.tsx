...
const pageTranslations = {
  serviceName: { ja: 'ヘレネクリニック 遠隔診療サービス', 'zh-TW': 'HELENE診所 遠程診斷服務', 'zh-CN': 'HELENE诊所 远程诊断服务', en: 'HELENE Clinic Remote Consultation' } as Record<Language, string>,
  serviceNameEn: { ja: 'HELENE Clinic - Remote Consultation', 'zh-TW': 'HELENE Clinic - Remote Consultation', 'zh-CN': 'HELENE Clinic - Remote Consultation', en: 'HELENE Clinic - Remote Consultation' } as Record<Language, string>,
  serviceDescription: { ja: 'ヘレネクリニック専門医とのビデオ診察・幹細胞治療プラン詳細説明・費用詳細見積', 'zh-TW': '與HELENE診所醫生遠程視頻會診、幹細胞治療方案評估、費用概算', 'zh-CN': '与HELENE诊所医生远程视频会诊、干细胞治疗方案评估、费用概算', en: 'Video consultation with HELENE doctor, stem cell treatment assessment, cost estimation' } as Record<Language, string>,
  serviceLongDescription: { ja: '表参道ヘレネクリニックの専門医と遠隔ビデオ診察を行い、専門医療通訳が全行程同行します。幹細胞治療の適応性、培養計画、治療スケジュール、費用明細等を詳しくご説明し、来日治療の決断をサポートします。', 'zh-TW': '與表參道HELENE診所專科醫生進行遠程視頻會診，由專業醫療翻譯全程陪同。醫生將詳細說明幹細胞治療的適用性、培養計劃、治療時程、費用明細等，幫助您做出赴日治療的決定。', 'zh-CN': '与表参道HELENE诊所专科医生进行远程视频会诊，由专业医疗翻译全程陪同。医生将详细说明干细胞治疗的适用性、培养计划、治疗时程、费用明细等，帮助您做出赴日治疗的决定。', en: 'Remote video consultation with Omotesando HELENE Clinic specialist, with professional medical interpreter. Doctor will explain stem cell therapy suitability, cultivation plan, treatment schedule, and cost details.' } as Record<Language, string>,
  ...
  feature1: { ja: 'ヘレネクリニック専門医とのビデオ診察（約30-60分）', 'zh-TW': 'HELENE診所專科醫生視頻會診（約30-60分鐘）', 'zh-CN': 'HELENE诊所专科医生视频会诊（约30-60分钟）', en: 'Video consultation with HELENE specialist (30-60 mins)' } as Record<Language, string>,
  ...
  feature3: { ja: '幹細胞治療の適応性評価', 'zh-TW': '幹細胞治療適用性評估', 'zh-CN': '干细胞治疗适用性评估', en: 'Stem cell therapy suitability assessment' } as Record<Language, string>,
  feature4: { ja: '個別培養計画と治療スケジュール説明', 'zh-TW': '個人培養計劃與治療時程說明', 'zh-CN': '个人培养计划与治疗时程说明', en: 'Personalized cultivation plan and schedule' } as Record<Language, string>,
  ...
  prereq3: { ja: 'ヘレネクリニックの初期評価完了', 'zh-TW': 'HELENE診所已完成初步評估', 'zh-CN': 'HELENE诊所已完成初步评估', en: 'HELENE Clinic completed initial assessment' } as Record<Language, string>,
  ...
  treatmentGoalPlaceholder: { ja: '例：膝関節痛の幹細胞治療、アンチエイジング等', 'zh-TW': '例如：膝關節幹細胞治療、抗衰老等', 'zh-CN': '例如：膝关节干细胞治疗、抗衰老等', en: 'e.g., Knee stem cell therapy, anti-aging, etc.' } as Record<Language, string>,
...
const SERVICE_INFO = {
  id: MEDICAL_PACKAGES['helene-remote-consultation'].slug,
  slug: MEDICAL_PACKAGES['helene-remote-consultation'].slug,
  price: MEDICAL_PACKAGES['helene-remote-consultation'].priceJpy,
};

export default function HeleneRemoteConsultationPage() {
  ...
  const [patientInfo, setPatientInfo] = useState({ patientName: '', age: '', gender: '', treatmentGoal: '', previousConsultation: '', questions: '' });
    ...
    <CheckoutLayout>
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-4xl mx-auto px-4 py-3">
          <Link href="/helene-clinic" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-indigo-600 transition">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <Video size={20} className="text-indigo-200" />
                <span className="text-indigo-200 text-sm">{t('videoConsultation')}</span>
              </div>
              <h1 className="text-3xl font-serif font-bold text-white">{t('serviceName')}</h1>
              <p className="text-indigo-200 text-sm mt-1">{t('serviceNameEn')}</p>
            </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1">
            <div className="bg-indigo-50 rounded-2xl p-6 border border-indigo-100 sticky top-8">
              <h3 className="text-lg font-bold text-indigo-900 mb-3">{t('featuresTitle')}</h3>
              ...
              <div className="space-y-2.5 text-sm text-gray-700">
                {[t('feature1'), t('feature2'), t('feature3'), t('feature4'), t('feature5'), t('feature6')].map((feature, idx) => (
                  <div key={idx} className="flex gap-2"><CheckCircle size={16} className="shrink-0 mt-0.5 text-indigo-500" /><span>{feature}</span></div>
                ))}
              </div>
              <div className="mt-6 pt-6 border-t border-indigo-200">
                <h4 className="font-bold text-gray-900 mb-3 flex items-center gap-2"><FileText size={16} className="text-indigo-600" />{t('prerequisitesTitle')}</h4>
                <ul className="space-y-2 text-sm text-gray-600">
                  {[t('prereq1'), t('prereq2'), t('prereq3')].map((prereq, idx) => (
                    <li key={idx} className="flex gap-2"><span className="text-indigo-400">•</span><span>{prereq}</span></li>
                  ))}
                </ul>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-2xl border border-gray-200 p-8">
              ...
              <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-4 mb-6">
                <p className="text-sm text-indigo-800"><strong>{t('contractNotice').split('：')[0]}：</strong>{t('contractNotice').split('：')[1]}</p>
              </div>

              <form onSubmit={handleSubmit} className="space-y-8">
                <div>
                  <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><Users size={18} className="text-indigo-600" />{t('patientInfoTitle')}</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('patientName')}</label>
                      <input type="text" required value={patientInfo.patientName} onChange={(e) => setPatientInfo({ ...patientInfo, patientName: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent" placeholder={t('patientNamePlaceholder')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('age')}</label>
                      <input type="text" value={patientInfo.age} onChange={(e) => setPatientInfo({ ...patientInfo, age: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent" placeholder={t('agePlaceholder')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('gender')}</label>
                      <select value={patientInfo.gender} onChange={(e) => setPatientInfo({ ...patientInfo, gender: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                    ...
                    <div>
                      ...
                      <input type="text" value={patientInfo.treatmentGoal} onChange={(e) => setPatientInfo({ ...patientInfo, treatmentGoal: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent" placeholder={t('treatmentGoalPlaceholder')} />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('previousConsultation')}</label>
                      <textarea value={patientInfo.previousConsultation} onChange={(e) => setPatientInfo({ ...patientInfo, previousConsultation: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent" rows={2} placeholder={t('previousConsultationPlaceholder')} />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('questionsToAsk')}</label>
                      <textarea value={patientInfo.questions} onChange={(e) => setPatientInfo({ ...patientInfo, questions: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent" rows={3} placeholder={t('questionsToAskPlaceholder')} />
                    </div>
                  </div>
                </div>

                <div>
                  <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><Clock size={18} className="text-indigo-600" />{t('preferredTimesTitle')}</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <input type="text" value={preferredTimes.time1} onChange={(e) => setPreferredTimes({ ...preferredTimes, time1: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent" placeholder={t('preferredTimePlaceholder1')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('preferredTime2')}</label>
                      <input type="text" value={preferredTimes.time2} onChange={(e) => setPreferredTimes({ ...preferredTimes, time2: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent" placeholder={t('preferredTimePlaceholder2')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('preferredTime3')}</label>
                      <input type="text" value={preferredTimes.time3} onChange={(e) => setPreferredTimes({ ...preferredTimes, time3: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent" placeholder={t('preferredTimePlaceholder3')} />
                    </div>
                  </div>
                </div>

                <div>
                  <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><Phone size={18} className="text-indigo-600" />{t('contactInfoTitle')}</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('contactName')}</label>
                      <input type="text" required value={customerInfo.name} onChange={(e) => setCustomerInfo({ ...customerInfo, name: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent" placeholder={t('contactNamePlaceholder')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('region')}</label>
                      <select value={customerInfo.country} onChange={(e) => setCustomerInfo({ ...customerInfo, country: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                    ...
                    <div>
                      ...
                      <input type="tel" value={customerInfo.phone} onChange={(e) => setCustomerInfo({ ...customerInfo, phone: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent" placeholder={t('phonePlaceholder')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('email')}</label>
                      <input type="email" value={customerInfo.email} onChange={(e) => setCustomerInfo({ ...customerInfo, email: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent" placeholder={t('emailPlaceholder')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('lineId')}</label>
                      <input type="text" value={customerInfo.line} onChange={(e) => setCustomerInfo({ ...customerInfo, line: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent" placeholder={t('lineIdPlaceholder')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('wechat')}</label>
                      <input type="text" value={customerInfo.wechat} onChange={(e) => setCustomerInfo({ ...customerInfo, wechat: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent" placeholder={t('wechatPlaceholder')} />
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent" rows={3} placeholder={t('additionalNotesPlaceholder')} />
                </div>
                ...
                <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
                  ...
                  <button type="submit" disabled={processing} className="w-full py-4 bg-gradient-to-r from-indigo-700 to-purple-800 text-white font-bold rounded-xl hover:from-indigo-800 hover:to-purple-900 transition shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                ...
                <div className="flex flex-wrap justify-center gap-6 text-xs text-gray-500">
                  ...
                  <div className="flex items-center gap-1"><Clock size={14} className="text-indigo-500" /><span>{t('confirmTime24h')}</span></div>
                </div>
              </form>
            </div>
          </div>
