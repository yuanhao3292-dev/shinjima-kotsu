...
const pageTranslations = {
  serviceName: { ja: 'ヘレネクリニック 初期相談サービス', 'zh-TW': 'HELENE診所 前期諮詢服務', 'zh-CN': 'HELENE诊所 前期咨询服务', en: 'HELENE Clinic Initial Consultation' } as Record<Language, string>,
  serviceNameEn: { ja: 'HELENE Clinic - Initial Consultation', 'zh-TW': 'HELENE Clinic - Initial Consultation', 'zh-CN': 'HELENE Clinic - Initial Consultation', en: 'HELENE Clinic - Initial Consultation' } as Record<Language, string>,
  serviceDescription: { ja: '病歴翻訳・クリニック相談・幹細胞治療プラン評価・費用概算', 'zh-TW': '病歷翻譯、診所諮詢、幹細胞治療方案評估、費用概算', 'zh-CN': '病历翻译、诊所咨询、干细胞治疗方案评估、费用概算', en: 'Medical record translation, clinic consultation, stem cell treatment assessment, cost estimate' } as Record<Language, string>,
  serviceLongDescription: { ja: '表参道ヘレネクリニック向けの初期相談サービス。診療情報を翻訳（中→日）し、クリニックと初期相談を行い、幹細胞治療の可能性を評価し、費用概算をご提供します。', 'zh-TW': '專為表參道HELENE診所提供的前期諮詢服務。我們將翻譯您的病歷資料（中→日），並與HELENE診所進行初步諮詢，評估幹細胞治療可行性並提供費用概算。', 'zh-CN': '专为表参道HELENE诊所提供的前期咨询服务。我们将翻译您的病历资料（中→日），并与HELENE诊所进行初步咨询，评估干细胞治疗可行性并提供费用概算。', en: 'Initial consultation for Omotesando HELENE Clinic. We translate your medical records (CN→JP), consult with the clinic, assess stem cell therapy feasibility and provide cost estimates.' } as Record<Language, string>,
  ...
  feature2: { ja: 'ヘレネクリニックへの初期相談', 'zh-TW': 'HELENE診所初步諮詢', 'zh-CN': 'HELENE诊所初步咨询', en: 'Initial consultation with HELENE Clinic' } as Record<Language, string>,
  feature3: { ja: '幹細胞治療可能性評価レポート', 'zh-TW': '幹細胞治療可行性評估報告', 'zh-CN': '干细胞治疗可行性评估报告', en: 'Stem cell therapy feasibility assessment' } as Record<Language, string>,
...
const SERVICE_INFO = {
  id: MEDICAL_PACKAGES['helene-initial-consultation'].slug,
  slug: MEDICAL_PACKAGES['helene-initial-consultation'].slug,
  price: MEDICAL_PACKAGES['helene-initial-consultation'].priceJpy,
};

export default function HeleneInitialConsultationPage() {
    ...
    <CheckoutLayout>
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-4xl mx-auto px-4 py-3">
          <Link href="/helene-clinic" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-blue-600 transition">
            <ArrowLeft size={16} />
            {t('backToMain')}
          </Link>
        </div>
              ...
              <div className="space-y-2.5 text-sm text-gray-700">
                {[t('feature1'), t('feature2'), t('feature3'), t('feature4'), t('feature5')].map((feature, idx) => (
                  <div key={idx} className="flex gap-2">
                    <CheckCircle size={16} className="shrink-0 mt-0.5 text-indigo-500" />
                    <span>{feature}</span>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="md:col-span-2">
                      ...
                      <textarea value={patientInfo.currentStatus} onChange={(e) => setPatientInfo({ ...patientInfo, currentStatus: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent" rows={3} placeholder={t('currentStatusPlaceholder')} />
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent" rows={4} placeholder={t('additionalNotesPlaceholder')} />
                </div>
                ...
                <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
                  ...
                  <button type="submit" disabled={processing} className="w-full py-4 bg-gradient-to-r from-gray-900 to-indigo-800 text-white font-bold rounded-xl hover:from-gray-800 hover:to-indigo-700 transition shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
