...
const pageTranslations = {
  serviceName: { ja: '近畿大学病院 遠隔診療サービス', 'zh-TW': '近畿大學醫院 遠程會診服務', 'zh-CN': '近畿大学医院 远程会诊服务', en: 'Kindai University Hospital Remote Consultation' } as Record<Language, string>,
  serviceNameEn: { ja: 'Kindai University Hospital - Remote Consultation', 'zh-TW': 'Kindai University Hospital - Remote Consultation', 'zh-CN': 'Kindai University Hospital - Remote Consultation', en: 'Kindai University Hospital - Remote Consultation' } as Record<Language, string>,
  serviceDescription: { ja: '近畿大学病院専門医とのビデオ診察・がん治療プラン策定・費用概算', 'zh-TW': '與近畿大學醫院專科醫生遠程視頻會診、癌症治療方案制定、費用概算', 'zh-CN': '与近畿大学医院专科医生远程视频会诊、癌症治疗方案制定、费用概算', en: 'Video consultation with Kindai University Hospital specialist, cancer treatment planning, cost estimation' } as Record<Language, string>,
  serviceLongDescription: { ja: '近畿大学病院の専門医と遠隔ビデオ診察を行い、専門医療通訳が全行程同行します。がんゲノム医療・集学的治療（MDT）の適応性、治療スケジュール、入院計画、費用明細等を詳しくご説明し、来日治療の決断をサポートします。', 'zh-TW': '與近畿大學醫院專科醫生進行遠程視頻會診，由專業醫療翻譯全程陪同。醫生將詳細說明癌症基因組醫療及多學科團隊（MDT）治療的適用性、治療時程、住院計劃、費用明細等，幫助您做出赴日治療的決定。', 'zh-CN': '与近畿大学医院专科医生进行远程视频会诊，由专业医疗翻译全程陪同。医生将详细说明癌症基因组医疗及多学科团队（MDT）治疗的适用性、治疗时程、住院计划、费用明细等，帮助您做出赴日治疗的决定。', en: 'Remote video consultation with Kindai University Hospital specialist, with professional medical interpreter. Doctor will explain cancer genomic medicine and multidisciplinary team (MDT) treatment suitability, treatment schedule, hospitalization plan, and cost details.' } as Record<Language, string>,
  ...
  backToMain: { ja: '近畿大学病院に戻る', 'zh-TW': '返回近畿大學醫院', 'zh-CN': '返回近畿大学医院', en: 'Back to Kindai University Hospital' } as Record<Language, string>,
  featuresTitle: { ja: 'サービス内容', 'zh-TW': '服務包含', 'zh-CN': '服务包含', en: 'Service Includes' } as Record<Language, string>,
  feature1: { ja: '近畿大学病院専門医とのビデオ診察（約30-60分）', 'zh-TW': '近畿大學醫院專科醫生視頻會診（約30-60分鐘）', 'zh-CN': '近畿大学医院专科医生视频会诊（约30-60分钟）', en: 'Video consultation with Kindai University Hospital specialist (30-60 mins)' } as Record<Language, string>,
  ...
  feature3: { ja: 'がんゲノム医療・先進治療の適応性評価', 'zh-TW': '癌症基因組醫療·先進治療適用性評估', 'zh-CN': '癌症基因组医疗·先进治疗适用性评估', en: 'Cancer genomic medicine & advanced treatment suitability assessment' } as Record<Language, string>,
  feature4: { ja: '集学的治療（MDT）に基づく個別治療計画', 'zh-TW': '基於多學科團隊（MDT）的個人治療計劃', 'zh-CN': '基于多学科团队（MDT）的个人治疗计划', en: 'Personalized treatment plan based on multidisciplinary team (MDT) approach' } as Record<Language, string>,
  feature5: { ja: '入院計画と費用明細見積', 'zh-TW': '住院計劃與費用明細報價', 'zh-CN': '住院计划与费用明细报价', en: 'Hospitalization plan and detailed cost quotation' } as Record<Language, string>,
  ...
  prereq2: { ja: '完全な診療情報・画像データ提出済み', 'zh-TW': '已提交完整病歷資料及影像數據', 'zh-CN': '已提交完整病历资料及影像数据', en: 'Submitted complete medical records and imaging data' } as Record<Language, string>,
  prereq3: { ja: '近畿大学病院の初期評価完了', 'zh-TW': '近畿大學醫院已完成初步評估', 'zh-CN': '近畿大学医院已完成初步评估', en: 'Kindai University Hospital completed initial assessment' } as Record<Language, string>,
  ...
  cancerType: { ja: 'がんの種類', 'zh-TW': '癌症類型', 'zh-CN': '癌症类型', en: 'Cancer Type' } as Record<Language, string>,
  cancerTypePlaceholder: { ja: '例：肝臓がん、膵臓がん、肺がん等', 'zh-TW': '例如：肝癌、胰腺癌、肺癌等', 'zh-CN': '例如：肝癌、胰腺癌、肺癌等', en: 'e.g., Liver cancer, pancreatic cancer, lung cancer, etc.' } as Record<Language, string>,
...
const SERVICE_INFO = {
  id: MEDICAL_PACKAGES['kindai-remote-consultation'].slug,
  slug: MEDICAL_PACKAGES['kindai-remote-consultation'].slug,
  price: MEDICAL_PACKAGES['kindai-remote-consultation'].priceJpy,
};

export default function KindaiRemoteConsultationPage() {
  ...
  const backHref = guideSlug ? `/g/${guideSlug}/kindai-hospital` : '/kindai-hospital';
    ...
    try {
      ...
      const response = await fetch('/api/create-checkout-session', {
        ...
        body: JSON.stringify({
          ...
          preferredTime: null,
          notes: fullNotes,
          provider: providerKey || 'kindai-hospital',
          locale: currentLang,
          ...(guideSlug ? { guideSlug } : {}),
        }),
      });
