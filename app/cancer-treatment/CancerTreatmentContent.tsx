...
const tr = {
  ...
  heroText: {
    ja: '2017年新築移転。都道府県がん診療連携拠点病院・がんゲノム医療拠点病院・\n特定機能病院。全がん5年相対生存率 69% — 全国トップクラスの治療実績。',
    'zh-TW': '2017年新建搬遷。都道府縣癌症診療中心·癌症基因組醫療中心·\n特定功能醫院。全癌五年相對生存率 69% — 全國頂尖治療實績。',
    'zh-CN': '2017年新建搬迁。都道府县癌症诊疗中心·癌症基因组医疗中心·\n特定功能医院。全癌五年相对生存率 69% — 全国顶尖治疗实绩。',
    en: 'Relocated to new facility in 2017. Designated Prefectural Cancer Hub,\nCancer Genome Medicine Hub & Special Function Hospital.\n5-year relative survival rate 69% — among the best in Japan.',
  } as Record<Language, string>,
  limitBadge: {
  ...
  // Common
  learnMore: { ja: '詳細を見る', 'zh-TW': '了解更多', 'zh-CN': '了解更多', en: 'Learn More' } as Record<Language, string>,
...
function CenterIcon({ type, className }: { type: string; className?: string }) {
  ...
  switch (type) {
    ...
    case 'rare': return <Gem className={cn} />;
...
// Component
// ======================================
export default function CancerTreatmentContent({ isGuideEmbed }: Props) {
  ...
  return (
    <div className={`min-h-screen bg-white ${isGuideEmbed ? '' : ''}`}>
      ...
      <section className="relative bg-gradient-to-br from-slate-900 via-blue-950 to-indigo-900 text-white overflow-hidden">
        ...
        <div className="relative container mx-auto px-6 py-20 md:py-28">
          <div className="max-w-3xl">
            ...
            <p className="text-sm text-blue-300 mb-4">{t('heroSubtitle')}</p>
            <p className="text-sm text-blue-200/80 whitespace-pre-line leading-relaxed mb-8">
              {t('heroText')}
            </p>

            {/* CTA */}
            <div className="flex flex-wrap gap-3">
              <a href="#services" className="inline-flex items-center gap-2 bg-white text-slate-900 font-bold px-6 py-3 rounded-xl hover:bg-blue-50 transition">
                <FileText size={18} />
                {t('consultPlan')}
              </a>
              <a href="#flow" className="inline-flex items-center gap-2 border border-white/30 text-white px-6 py-3 rounded-xl hover:bg-white/10 transition">
                <ArrowRight size={18} />
                {t('viewFlow')}
              </a>
            </div>
          ...
          <div className="mt-12 grid grid-cols-1 sm:grid-cols-3 gap-3">
            {[
              { icon: Shield, text: { ja: '特定機能病院・がんゲノム医療拠点', 'zh-TW': '特定功能醫院·基因組醫療中心', 'zh-CN': '特定功能医院·基因组医疗中心', en: 'Special Function Hospital & Genome Hub' } as Record<Language, string> },
              { icon: Award, text: { ja: '大阪府がん診療連携拠点病院', 'zh-TW': '大阪府癌症診療中心', 'zh-CN': '大阪府癌症诊疗中心', en: 'Prefectural Cancer Diagnosis Hub' } as Record<Language, string> },
              { icon: Users, text: { ja: '大阪府66施設のがん登録データ統括', 'zh-TW': '統括大阪府66家機構癌症數據', 'zh-CN': '统括大阪府66家机构癌症数据', en: 'Cancer registry data from 66 Osaka facilities' } as Record<Language, string> },
            ].map((badge, i) => (
              <div key={i} className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl px-4 py-3">
                <badge.icon size={18} className="text-blue-300 flex-shrink-0" />
                <span className="text-xs text-blue-100">{badge.text[lang]}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 bg-gray-50">
        <div className="container mx-auto px-6">
          <div className="text-center mb-12">
            <span className="text-blue-600 text-xs tracking-widest uppercase font-bold">{t('statsTag')}</span>
            <h2 className="text-3xl font-bold text-gray-900 mt-2">{t('statsTitle')}</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {KEY_STATS.map((stat, i) => (
              <div key={i} className="bg-white rounded-2xl p-5 shadow-sm hover:shadow-md transition text-center">
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-6">
          <div className="text-center mb-14">
            <span className="text-indigo-600 text-xs tracking-widest uppercase font-bold">{t('centersTag')}</span>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
            {SPECIALTY_CENTERS.map((center, i) => (
              <div key={i} className="group bg-white border border-gray-200 rounded-2xl overflow-hidden hover:shadow-lg transition-all">
                {/* Top gradient bar */}
                <div className={`h-1.5 bg-gradient-to-r ${center.gradient}`} />
                <div className="p-5">
                  <div className="flex items-center gap-3 mb-3">
                    <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${center.gradient} flex items-center justify-center`}>
                      <CenterIcon type={center.icon} className="w-5 h-5 text-white" />
                    </div>
                  ...
                  <div className="flex items-center gap-1.5 text-xs">
                    ...
                    <span className="text-amber-700 font-medium">{center.stats[lang]}</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="py-20 bg-gradient-to-b from-gray-50 to-white">
        <div className="container mx-auto px-6">
          <div className="text-center mb-14">
            <span className="text-emerald-600 text-xs tracking-widest uppercase font-bold">{t('treatTag')}</span>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {TREATMENTS.map((treat, i) => (
              <div key={i} className="bg-white rounded-2xl border border-gray-100 p-6 hover:shadow-lg transition-all">
                <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${treat.gradient} flex items-center justify-center mb-4`}>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-6">
          <div className="text-center mb-14">
            <span className="text-rose-600 text-xs tracking-widest uppercase font-bold">{t('deptsTag')}</span>
            <h2 className="text-3xl font-bold text-gray-900 mt-2">{t('deptsTitle')}</h2>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {DEPARTMENTS.map((group, i) => (
              <div key={i} className={`rounded-2xl border ${group.color} p-6`}>
                <h3 className={`font-bold ${group.textColor} mb-4`}>{group.category[lang]}</h3>
                <ul className="space-y-2">
                  {group.depts.map((dept, j) => (
                    <li key={j} className="flex items-start gap-2 text-sm text-gray-700">
                      <CheckCircle size={14} className={`${group.textColor} mt-0.5 flex-shrink-0`} />
                      <span>
      ...
      <section id="flow" className="py-20 bg-gray-50">
        <div className="container mx-auto px-6">
          <div className="text-center mb-14">
            <span className="text-purple-600 text-xs tracking-widest uppercase font-bold">{t('flowTag')}</span>
                  ...
                  {phase.steps.map((step, j) => (
                    <div key={j} className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
                      <div className="flex items-start gap-4">
                        <div className={`w-8 h-8 rounded-full ${phase.color} text-white flex items-center justify-center font-bold text-sm flex-shrink-0`}>
                          {j + 1}
                        </div>
                        <div>
      ...
      <section id="services" className="py-20 bg-white">
        <div className="container mx-auto px-6">
          <div className="text-center mb-14">
            <span className="text-blue-600 text-xs tracking-widest uppercase font-bold">{t('svcTag')}</span>
          ...
          <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {packages.map((pkg) => {
              ...
              return (
                <div key={pkg.slug} className="bg-white rounded-2xl border-2 border-gray-200 hover:border-blue-300 p-6 transition shadow-sm hover:shadow-md">
                  <h3 className="text-lg font-bold text-gray-900 mb-2">{name}</h3>
                  <div className="flex items-baseline gap-1 mb-4">
                    <span className="text-3xl font-black text-blue-600">¥{pkg.priceJpy.toLocaleString()}</span>
                  ...
                  <Link
                    ...
                    className="block w-full text-center bg-blue-600 text-white font-bold py-3 rounded-xl hover:bg-blue-700 transition"
                  >
                    {t('bookNow')}
                  </Link>
                </div>
            ...
            <div className="grid sm:grid-cols-2 gap-4 text-sm text-gray-600">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <a href="https://oici.jp" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline flex items-center gap-1">
                    oici.jp <ExternalLink size={12} />
                  </a>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <Train size={16} className="text-gray-400 mt-0.5 flex-shrink-0" />
                <div>
                  <p className="font-medium text-gray-800">
        ...
        <div className="container mx-auto px-6 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            ...
            <a
              ...
              className="inline-flex items-center gap-2 bg-gray-800 text-white font-bold px-6 py-3 rounded-xl hover:bg-gray-700 transition"
            >
              <Mail size={18} />
              Email
            </a>
