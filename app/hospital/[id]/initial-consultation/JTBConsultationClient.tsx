...
// ─── 翻译（与直营医院咨询页完全一致，仅医院名动态替换） ───

const pageTranslations = {
  serviceNameSuffix: { ja: '初期相談サービス', 'zh-TW': '前期諮詢服務', 'zh-CN': '前期咨询服务', en: 'Initial Consultation' } as Record<Language, string>,
  ...
  backToScreening: { ja: 'AI筛查結果に戻る', 'zh-TW': '返回AI篩查報告', 'zh-CN': '返回AI筛查报告', en: 'Back to Screening Report' } as Record<Language, string>,
...
const PACKAGE_SLUG = 'jtb-initial-consultation';

function getHospitalName(hospital: JTBHospitalData, lang: Language): string {
  switch (lang) {
    case 'ja': return hospital.name_ja;
    case 'zh-TW': return hospital.name_zh_tw || hospital.name_zh_cn || hospital.name_ja;
    case 'zh-CN': return hospital.name_zh_cn || hospital.name_ja;
    case 'en': return hospital.name_en || hospital.name_ja;
    default: return hospital.name_ja;
  }
}

export default function JTBConsultationClient({ hospital }: { hospital: JTBHospitalData }) {
  ...
  const [patientInfo, setPatientInfo] = useState({
    patientName: '', age: '', gender: '', diagnosis: '', currentStatus: '',
  });
  ...
  const hospitalName = getHospitalName(hospital, currentLang);
  const serviceName = `${hospitalName} ${t('serviceNameSuffix')}`;
  ...
  async function handleConfirmedSubmit() {
    ...
    try {
      ...
      let fullNotes = `【${serviceName}】\n`;
      fullNotes += `Hospital: ${hospital.name_ja} (JTB ID: ${hospital.jtb_id || 'N/A'})\n\n`;
      ...
      const response = await fetch('/api/create-checkout-session', {
        ...
        body: JSON.stringify({
          ...
          preferredTime: null,
          notes: fullNotes,
          provider: providerKey || `jtb_${hospital.jtb_id || hospital.id}`,
          locale: currentLang,
          consents,
          ...(guideSlug ? { guideSlug } : {}),
        }),
    ...
    <CheckoutLayout>
      ...
      <div className="bg-white border-b border-neutral-100">
        <div className="max-w-4xl mx-auto px-4 py-3">
          <Link href={(searchParams.get('from')?.startsWith('/') ? searchParams.get('from') : null) || '/health-screening/history'} className="inline-flex items-center gap-2 text-sm text-neutral-500 hover:text-brand-700 transition">
            <ArrowLeft size={16} />
            {t('backToScreening')}
          </Link>
        </div>
