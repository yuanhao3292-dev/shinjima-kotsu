...
function BookingsContent() {
  ...
  const getDepositBadge = (status: string) => {
    ...
    return (
      <span className={`px-2 py-1 rounded-md text-xs border ${styles[status] || styles.pending}`}>
        {labelKeys[status] ? t(labelKeys[status], lang) : status}
      </span>
    );
  };
        ...
        <PageMotion className="p-6 lg:p-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 bg-zinc-900 hover:bg-zinc-800 text-white font-medium px-4 py-2 rounded-lg transition"
            >
              <Plus size={18} />
              {t('newBooking', lang)}
            </Link>
          ...
          <div className="space-y-4">
            {filteredBookings.map((booking) => (
              <div
                key={booking.id}
                className="bg-white rounded-xl border border-zinc-200 shadow-sm hover:border-zinc-300 transition overflow-hidden"
              >
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                    ...
                    <div className="flex-grow">
                      <div className="flex items-center gap-3 mb-2">
                        <h3 className="font-bold text-zinc-900 text-lg">{booking.customer_name}</h3>
                    ...
                    <div className="sm:text-right">
                      {booking.actual_spend && (
                        <div className="mb-2">
                          <p className="text-xs text-zinc-500">{t('actualSpend', lang)}</p>
                          <p className="font-bold text-zinc-900">¥{booking.actual_spend.toLocaleString()}</p>
                        </div>
                      )}
                      {booking.commission_amount && (
                        <div>
                          <p className="text-xs text-zinc-500">{t('commission', lang)}</p>
                ...
                {booking.status === 'pending' && booking.deposit_status === 'pending' && (
                  <div className="px-4 sm:px-6 py-3 border-t border-zinc-200">
                    <button
                ...
                <div className="px-4 sm:px-6 py-3 bg-zinc-50 border-t border-zinc-200 flex items-center justify-between">
                  ...
                  <Link
                    ...
                    className="text-zinc-500 text-sm font-medium flex items-center gap-1 hover:underline"
                  >
          ...
          {filteredBookings.length === 0 && (
            <div className="bg-white rounded-xl border border-zinc-200 shadow-sm p-12 text-center">
              ...
              <h3 className="font-bold text-zinc-900 mb-2">{t('noBookings', lang)}</h3>
              <p className="text-zinc-500 mb-4">{t('startBooking', lang)}</p>
              <Link
                href="/guide-partner/venues"
                className="inline-flex items-center gap-2 bg-zinc-900 hover:bg-zinc-800 text-white font-medium px-6 py-3 rounded-lg transition"
              >
                <Store size={18} />
                {t('browseVenues', lang)}
              </Link>
