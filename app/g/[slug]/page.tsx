...
export default async function GuideDistributionPage({ params }: PageProps) {
  ...
  const genericModules = selectedModules.filter((m) => {
    ...
    return true; // 没有 displayConfig 的归入通用卡片
  });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      >
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            {guide.brandLogoUrl ? (
              <img src={guide.brandLogoUrl} alt={brandName} className="h-10 w-auto" />
            ) : (
              <div
      ...
      >
        <div className="max-w-6xl mx-auto px-6 text-center">
          ...
          <p className="text-xl text-white/80 max-w-2xl mx-auto">
            {config.seoDescription || '专业日本高端医疗体检、商务考察、私人定制服务。中文服务、专属定制。'}
          </p>
        </div>
      </section>
      ...
      <section id="about" className="py-16">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-12">
            ...
            <p className="text-gray-600 max-w-2xl mx-auto">为您提供最优质的日本旅行和医疗服务体验</p>
          </div>
          <div className="bg-white rounded-2xl shadow-lg p-8">
            <div className="flex flex-col md:flex-row items-center gap-8">
              <div className="w-32 h-32 rounded-full bg-gray-100 flex items-center justify-center">
                {guide.brandLogoUrl ? (
                  <img src={guide.brandLogoUrl} alt={brandName} className="w-24 h-24 object-contain" />
                ) : (
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                <p className="text-gray-600 mb-4">专业导游，为您提供最贴心的服务。</p>
                <div className="flex flex-wrap justify-center md:justify-start gap-3">
                  <span className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-sm">中文服务</span>
                  <span className="px-3 py-1 bg-green-100 text-green-700 rounded-full text-sm">专属定制</span>
                  <span className="px-3 py-1 bg-purple-100 text-purple-700 rounded-full text-sm">高端体验</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <section id="vehicles" className="py-16 bg-white">
          <div className="max-w-6xl mx-auto px-6">
            <div className="text-center mb-12">
              ...
              <p className="text-gray-600 max-w-2xl mx-auto">为您提供舒适、安全的高端座驾</p>
            </div>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {selectedVehicles.map((sv) => (
                <div key={sv.id} className="bg-gray-50 rounded-xl overflow-hidden border hover:shadow-lg transition">
                  <div className="aspect-video bg-gray-200 flex items-center justify-center">
                    {sv.vehicle.images?.[0] ? (
                      <img src={sv.vehicle.images[0]} alt={sv.vehicle.name} className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <div className="p-5">
                    ...
                    <p className="text-sm text-gray-600 mb-3">{sv.vehicle.description || sv.vehicle.vehicleType}</p>
                    <div className="flex items-center gap-4 text-sm text-gray-500">
                      <span className="flex items-center gap-1"><Users size={14} />{sv.vehicle.seats} 座</span>
                    </div>
                    {sv.vehicle.features && sv.vehicle.features.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-3">
                        {sv.vehicle.features.slice(0, 3).map((feature, idx) => (
                          <span key={idx} className="px-2 py-0.5 bg-gray-200 text-gray-600 text-xs rounded">{feature}</span>
                        ))}
                      </div>
                    )}
                  </div>
      ...
      {/* 通用服务卡片（没有 displayConfig 的模块） */}
      {genericModules.length > 0 && (
        <section id="services" className="py-16">
          <div className="max-w-6xl mx-auto px-6">
            <div className="text-center mb-12">
              <h2 className="text-3xl font-bold text-gray-900 mb-4">服务项目</h2>
              <p className="text-gray-600 max-w-2xl mx-auto">为您精选的优质医疗和旅行服务</p>
            </div>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {genericModules.map((sm) => (
                <div key={sm.id} className="bg-white rounded-xl border overflow-hidden hover:shadow-lg transition group">
                  <div className="p-6">
                    <div className="flex items-center gap-4 mb-4">
                      <div className="w-14 h-14 rounded-xl flex items-center justify-center" style={{ backgroundColor: `${brandColor}15` }}>
                    ...
                    <p className="text-gray-600 text-sm mb-4 line-clamp-3">{sm.module.description || '优质服务项目'}</p>
                    <a
                      href="#contact"
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-3">
              {guide.brandLogoUrl ? (
                ...
                <div className="w-8 h-8 rounded-lg bg-white/20 flex items-center justify-center font-bold">{brandName.charAt(0)}</div>
              )}
