...
const pageTranslations = {
  serviceName: { ja: '兵庫医大 初期相談サービス', 'zh-TW': '兵庫醫大 前期諮詢服務', 'zh-CN': '兵库医大 前期咨询服务', en: 'Hyogo Medical Initial Consultation' } as Record<Language, string>,
  serviceNameEn: { ja: 'Hyogo Medical - Initial Consultation', 'zh-TW': 'Hyogo Medical - Initial Consultation', 'zh-CN': 'Hyogo Medical - Initial Consultation', en: 'Hyogo Medical - Initial Consultation' } as Record<Language, string>,
  serviceDescription: { ja: '資料翻訳・兵庫医大相談・治療プラン初期評価', 'zh-TW': '資料翻譯、兵庫醫大諮詢、治療方案初步評估', 'zh-CN': '资料翻译、兵库医大咨询、治疗方案初步评估', en: 'Document translation, Hyogo Medical consultation, initial treatment assessment' } as Record<Language, string>,
  serviceLongDescription: { ja: '兵庫医科大学病院向けの初期相談サービス。診療情報を翻訳（中→日）し、兵庫医大と初期相談を行い、治療可能性を評価し、費用概算をご提供します。', 'zh-TW': '專為兵庫醫科大學病院提供的前期諮詢服務。我們將翻譯您的病歷資料（中文→日文），並與兵庫醫大進行初步諮詢，評估治療可行性並提供費用概算。', 'zh-CN': '专为兵库医科大学病院提供的前期咨询服务。我们将翻译您的病历资料（中文→日文），并与兵库医大进行初步咨询，评估治疗可行性并提供费用概算。', en: 'Initial consultation service for Hyogo Medical University Hospital. We translate your medical records (CN→JP), consult with Hyogo Medical, assess treatment feasibility and provide cost estimates.' } as Record<Language, string>,
  ...
  feature2: { ja: '兵庫医大への初期相談', 'zh-TW': '兵庫醫大初步諮詢', 'zh-CN': '兵库医大初步咨询', en: 'Initial consultation with Hyogo Medical' } as Record<Language, string>,
    ...
    <CheckoutLayout>
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-4xl mx-auto px-4 py-3">
          <Link href="/hyogo-medical" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-blue-600 transition">
            <ArrowLeft size={16} />
            {t('backToMain')}
          </Link>
        </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
                  ...
                  <button type="submit" disabled={processing} className="w-full py-4 bg-gradient-to-r from-blue-600 to-indigo-700 text-white font-bold rounded-xl hover:from-blue-700 hover:to-indigo-800 transition shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
