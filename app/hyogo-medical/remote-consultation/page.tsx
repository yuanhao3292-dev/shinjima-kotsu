...
const pageTranslations = {
  serviceName: { ja: '兵庫医大 遠隔診療サービス', 'zh-TW': '兵庫醫大 遠程會診服務', 'zh-CN': '兵库医大 远程会诊服务', en: 'Hyogo Medical Remote Consultation' } as Record<Language, string>,
  serviceNameEn: { ja: 'Hyogo Medical - Remote Consultation', 'zh-TW': 'Hyogo Medical - Remote Consultation', 'zh-CN': 'Hyogo Medical - Remote Consultation', en: 'Hyogo Medical - Remote Consultation' } as Record<Language, string>,
  serviceDescription: { ja: '兵庫医大専門医とのビデオ診察・治療プラン詳細説明・費用詳細見積', 'zh-TW': '與兵庫醫大醫生遠程視頻會診、討論治療方案、費用概算', 'zh-CN': '与兵库医大医生远程视频会诊、讨论治疗方案、费用概算', en: 'Video consultation with Hyogo Medical doctor, treatment plan discussion, cost estimation' } as Record<Language, string>,
  serviceLongDescription: { ja: '兵庫医大の専門医と遠隔ビデオ診察を行い、専門医療通訳が全行程同行します。医師は治療計画、期待される効果、治療費用の詳細等を詳しく説明し、来日治療の決断をサポートします。', 'zh-TW': '與兵庫醫大專科醫生進行遠程視頻會診，由專業醫療翻譯全程陪同。醫生將詳細說明治療方案、預期效果、治療費用明細等，幫助您做出赴日治療的決定。', 'zh-CN': '与兵库医大专科医生进行远程视频会诊，由专业医疗翻译全程陪同。医生将详细说明治疗方案、预期效果、治疗费用明细等，帮助您做出赴日治疗的决定。', en: 'Remote video consultation with Hyogo Medical specialist, accompanied by professional medical interpreter. Doctor will explain treatment plan, expected outcomes, and detailed costs.' } as Record<Language, string>,
  ...
  backToMain: { ja: '兵庫医科大学病院に戻る', 'zh-TW': '返回兵庫醫大', 'zh-CN': '返回兵库医大', en: 'Back to Hyogo Medical' } as Record<Language, string>,
  featuresTitle: { ja: 'サービス内容', 'zh-TW': '服務包含', 'zh-CN': '服务包含', en: 'Service Includes' } as Record<Language, string>,
  feature1: { ja: '兵庫医大専門医とのビデオ診察（約30-60分）', 'zh-TW': '兵庫醫大專科醫生視頻會診（約30-60分鐘）', 'zh-CN': '兵库医大专科医生视频会诊（约30-60分钟）', en: 'Video consultation with Hyogo Medical specialist (30-60 mins)' } as Record<Language, string>,
  ...
  prereq3: { ja: '兵庫医大の初期評価完了', 'zh-TW': '兵庫醫大已完成初步評估', 'zh-CN': '兵库医大已完成初步评估', en: 'Hyogo Medical completed initial assessment' } as Record<Language, string>,
    ...
    <CheckoutLayout>
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-4xl mx-auto px-4 py-3">
          <Link href="/hyogo-medical" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-purple-600 transition">
            ...
            <div className="bg-purple-50 rounded-2xl p-6 border border-purple-100 sticky top-8">
              ...
              <div className="space-y-2.5 text-sm text-gray-700">
                {[t('feature1'), t('feature2'), t('feature3'), t('feature4'), t('feature5'), t('feature6')].map((feature, idx) => (
                  <div key={idx} className="flex gap-2"><CheckCircle size={16} className="shrink-0 mt-0.5 text-purple-500" /><span>{feature}</span></div>
                ))}
              </div>
              <div className="mt-6 pt-6 border-t border-purple-200">
                <h4 className="font-bold text-gray-900 mb-3 flex items-center gap-2"><FileText size={16} className="text-purple-600" />{t('prerequisitesTitle')}</h4>
                <ul className="space-y-2 text-sm text-gray-600">
                  {[t('prereq1'), t('prereq2'), t('prereq3')].map((prereq, idx) => (
                    <li key={idx} className="flex gap-2"><span className="text-purple-400">•</span><span>{prereq}</span></li>
                  ))}
                </ul>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-2xl border border-gray-200 p-8">
              ...
              <div className="bg-purple-50 border border-purple-200 rounded-xl p-4 mb-6">
                <p className="text-sm text-purple-800"><strong>{t('contractNotice').split('：')[0]}：</strong>{t('contractNotice').split('：')[1]}</p>
              </div>

              <form onSubmit={handleSubmit} className="space-y-8">
                <div>
                  <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><Users size={18} className="text-purple-600" />{t('patientInfoTitle')}</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('patientName')}</label>
                      <input type="text" required value={patientInfo.patientName} onChange={(e) => setPatientInfo({ ...patientInfo, patientName: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent" placeholder={t('patientNamePlaceholder')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('age')}</label>
                      <input type="text" value={patientInfo.age} onChange={(e) => setPatientInfo({ ...patientInfo, age: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent" placeholder={t('agePlaceholder')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('gender')}</label>
                      <select value={patientInfo.gender} onChange={(e) => setPatientInfo({ ...patientInfo, gender: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                    ...
                    <div>
                      ...
                      <input type="text" value={patientInfo.diagnosis} onChange={(e) => setPatientInfo({ ...patientInfo, diagnosis: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent" placeholder={t('diagnosisPlaceholder')} />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('previousConsultation')}</label>
                      <textarea value={patientInfo.previousConsultation} onChange={(e) => setPatientInfo({ ...patientInfo, previousConsultation: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent" rows={2} placeholder={t('previousConsultationPlaceholder')} />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('questionsToAsk')}</label>
                      <textarea value={patientInfo.questions} onChange={(e) => setPatientInfo({ ...patientInfo, questions: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent" rows={3} placeholder={t('questionsToAskPlaceholder')} />
                    </div>
                  </div>
                </div>

                <div>
                  <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><Clock size={18} className="text-purple-600" />{t('preferredTimesTitle')}</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <input type="text" value={preferredTimes.time1} onChange={(e) => setPreferredTimes({ ...preferredTimes, time1: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent" placeholder={t('preferredTimePlaceholder1')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('preferredTime2')}</label>
                      <input type="text" value={preferredTimes.time2} onChange={(e) => setPreferredTimes({ ...preferredTimes, time2: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent" placeholder={t('preferredTimePlaceholder2')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('preferredTime3')}</label>
                      <input type="text" value={preferredTimes.time3} onChange={(e) => setPreferredTimes({ ...preferredTimes, time3: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent" placeholder={t('preferredTimePlaceholder3')} />
                    </div>
                  </div>
                </div>

                <div>
                  <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><Phone size={18} className="text-purple-600" />{t('contactInfoTitle')}</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('contactName')}</label>
                      <input type="text" required value={customerInfo.name} onChange={(e) => setCustomerInfo({ ...customerInfo, name: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent" placeholder={t('contactNamePlaceholder')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('region')}</label>
                      <select value={customerInfo.country} onChange={(e) => setCustomerInfo({ ...customerInfo, country: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                    ...
                    <div>
                      ...
                      <input type="tel" value={customerInfo.phone} onChange={(e) => setCustomerInfo({ ...customerInfo, phone: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent" placeholder={t('phonePlaceholder')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('email')}</label>
                      <input type="email" value={customerInfo.email} onChange={(e) => setCustomerInfo({ ...customerInfo, email: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent" placeholder={t('emailPlaceholder')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('lineId')}</label>
                      <input type="text" value={customerInfo.line} onChange={(e) => setCustomerInfo({ ...customerInfo, line: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent" placeholder={t('lineIdPlaceholder')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('wechat')}</label>
                      <input type="text" value={customerInfo.wechat} onChange={(e) => setCustomerInfo({ ...customerInfo, wechat: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent" placeholder={t('wechatPlaceholder')} />
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent" rows={3} placeholder={t('additionalNotesPlaceholder')} />
                </div>
                ...
                <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
                  ...
                  <button type="submit" disabled={processing} className="w-full py-4 bg-gradient-to-r from-purple-600 to-pink-700 text-white font-bold rounded-xl hover:from-purple-700 hover:to-pink-800 transition shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                ...
                <div className="flex flex-wrap justify-center gap-6 text-xs text-gray-500">
                  ...
                  <div className="flex items-center gap-1"><Clock size={14} className="text-purple-500" /><span>{t('confirmTime24h')}</span></div>
                </div>
              </form>
            </div>
          </div>
