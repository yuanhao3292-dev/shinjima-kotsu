...
const INJECTABLE_PACKAGES = [
  ...
  { slug: 'sai-exosome-therapy', name: '干细胞外泌体疗法', nameJa: 'エクソソーム療法', price: 760000, desc: '新�的干细胞外泌体（2-3次疗程），最前沿再生医疗' },
];
...
const REVIEWS = [
  { name: '王小姐', loc: '上海', flag: '🇨🇳', treatment: 'SAI LIFT STANDARD', text: '做了线雕提升，效果非常自然！崔医生很有经验，整个过程很安心。术后恢复也很快，朋友都说我变年轻了但看不出做了手术。', highlight: '效果自然、恢复快' },
  { name: '陳小姐', loc: '台北', flag: '🇹🇼', treatment: '双眼皮+法令纹套餐', text: '从台北专程来大阪做的双眼皮和法令纹。崔医生的审美很好，非常自然的韩式风格。诊所在梅田地下街，交通超方便。', highlight: '韩式审美、交通便利' },
  { name: '李先生', loc: '深圳', flag: '🇨🇳', treatment: '玻尿酸+肉毒素', text: '第一次在日本做医美，原本很紧张。但诊所的环境很温馨，医生也非常耐心地解释了所有细节。价格合理，效果满意。', highlight: '服务贴心、价格透明' },
  { name: '張小姐', loc: '香港', flag: '🇭🇰', treatment: 'SAI LIFT PERFECT', text: '试过很多家诊所的线雕，SAI CLINIC的提升效果是最好的。崔医生的技术真的很精湛，而且会根据你的面部特征做个性化设计。', highlight: '技术精湛、个性化' },
  { name: '黄小姐', loc: '北京', flag: '🇨🇳', treatment: '水光+干细胞', text: '做了水光注射加干细胞精华，皮肤状态明显改善。崔医生很专业，还给我做了营养分析，从内到外调理。下次还会来。', highlight: '内外兼修、效果显著' },
  { name: '林先生', loc: '新竹', flag: '🇹🇼', treatment: 'AGA生发治疗', text: '之前一直在纠结要不要做生发治疗，来到SAI CLINIC后崔医生给了很中肯的建议。鸡尾酒注射效果不错，头发明显变浓密了。', highlight: '建议中肯、效果可见' },
];
...
export default function SaiClinicContent({ whitelabel }: SaiClinicContentProps) {
  ...
  return (
    <div className="animate-fade-in-up">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6 py-24">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="text-xl text-rose-200/80 mb-4 font-light">抗衰老 × 韩式美学 × 精准医美</p>
              <p className="text-base text-gray-300/80 leading-relaxed mb-8 max-w-lg">
                以糸リフト（线雕提升）为核心，融合最新韩国美容趋势与日本精密医疗。
                崔煌植医生亲诊，为每位客人量身定制自然美丽方案。
              </p>
            ...
            {THREAD_LIFT_PACKAGES.map((pkg) => (
              ...
              >
                ...
                <div className="mb-4">
                  ...
                  <p className={`text-[10px] ${pkg.flagship ? 'text-gray-500' : 'text-gray-400'}`}>含税·含全程服务费</p>
                </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {COMBO_PACKAGES.map((pkg) => (
              <div key={pkg.slug} className="bg-white rounded-2xl p-6 border border-gray-100 hover:shadow-xl transition hover:-translate-y-1 flex flex-col">
                ...
                <p className="text-xl font-bold text-gray-900 mb-3">{formatPrice(pkg.price)}</p>
                <Link href={`${checkoutBase}/${pkg.slug}`} className="w-full py-2 border border-rose-200 text-rose-600 text-sm font-bold rounded-lg hover:bg-rose-50 transition text-center block">预约咨询</Link>
              </div>
            ))}
          </div>
        </div>
                ...
                {EYE_PACKAGES.map((pkg) => (
                  <div key={pkg.slug} className="bg-purple-50/50 rounded-xl p-5 border border-purple-100 hover:shadow-lg transition flex flex-col sm:flex-row sm:items-center gap-4">
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <Link href={`${checkoutBase}/${pkg.slug}`} className="inline-block mt-2 px-4 py-1.5 bg-purple-600 text-white text-xs font-bold rounded-lg hover:bg-purple-700 transition">预约</Link>
                    </div>
                  </div>
                ))}
              </div>
                ...
                {NOSE_PACKAGES.map((pkg) => (
                  <div key={pkg.slug} className="bg-blue-50/50 rounded-xl p-5 border border-blue-100 hover:shadow-lg transition flex flex-col sm:flex-row sm:items-center gap-4">
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <Link href={`${checkoutBase}/${pkg.slug}`} className="inline-block mt-2 px-4 py-1.5 bg-blue-600 text-white text-xs font-bold rounded-lg hover:bg-blue-700 transition">预约</Link>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {[...INJECTABLE_PACKAGES, ...FAT_PACKAGES, ...WELLNESS_PACKAGES].map((pkg) => (
              <div key={pkg.slug} className="bg-white rounded-2xl p-6 border border-gray-100 hover:shadow-xl transition hover:-translate-y-1 flex flex-col">
                ...
                <div className="flex items-end justify-between">
                  ...
                  <Link href={`${checkoutBase}/${pkg.slug}`} className="px-4 py-2 border border-rose-200 text-rose-600 text-xs font-bold rounded-lg hover:bg-rose-50 transition">预约</Link>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-24 bg-gray-50 overflow-hidden">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl font-bold text-gray-900 mt-3">客户好评</h2>
          </div>
        </div>
          ...
          <div className="flex gap-6 animate-scroll-reviews px-6">
            {[...REVIEWS, ...REVIEWS].map((r, i) => (
              <div key={i} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm min-w-[320px] max-w-[320px] shrink-0">
                ...
                <p className="text-xs text-rose-500 font-medium">{r.highlight}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 bg-gradient-to-r from-rose-600 to-pink-600 text-white text-center">
        <div className="max-w-3xl mx-auto px-6">
          <h2 className="text-3xl md:text-4xl font-bold mb-6">开启您的美丽之旅</h2>
          <p className="text-rose-100 text-lg mb-10 leading-relaxed">
            崔煌植医生亲自为您制定专属方案。
            从线上咨询到术后跟进，全程中文服务。
          </p>
          <a href="#sai-packages" className="inline-flex items-center gap-3 bg-white text-rose-600 px-10 py-4 rounded-full font-bold hover:bg-rose-50 transition-all shadow-lg">
            查看全部项目 <ArrowRight size={18} />
          </a>
        </div>
      </section>
