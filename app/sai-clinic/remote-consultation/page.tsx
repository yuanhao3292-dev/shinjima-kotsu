...
const pageTranslations = {
  serviceName: { ja: 'SAI CLINIC 遠隔相談サービス', 'zh-TW': 'SAI CLINIC 遠程諮詢服務', 'zh-CN': 'SAI CLINIC 远程咨询服务', en: 'SAI CLINIC Remote Consultation' } as Record<Language, string>,
  serviceNameEn: { ja: 'SAI CLINIC - Remote Consultation', 'zh-TW': 'SAI CLINIC - Remote Consultation', 'zh-CN': 'SAI CLINIC - Remote Consultation', en: 'SAI CLINIC - Remote Consultation' } as Record<Language, string>,
  serviceDescription: { ja: 'SAI CLINIC専門医とのビデオ相談・美容整形施術プラン策定・費用概算', 'zh-TW': '與SAI CLINIC專科醫生遠程視頻諮詢、醫美整形療程規劃、費用概算', 'zh-CN': '与SAI CLINIC专科医生远程视频咨询、医美整形疗程规划、费用概算', en: 'Video consultation with SAI CLINIC specialist, cosmetic procedure planning, cost estimation' } as Record<Language, string>,
  serviceLongDescription: { ja: 'SAI CLINIC専門医と遠隔ビデオ相談を行い、専門医療通訳が全行程同行します。糸リフト・二重整形・ヒアルロン酸注入等の適応性、施術スケジュール、ダウンタイム、費用明細等を詳しくご説明し、来日施術の決断をサポートします。', 'zh-TW': '與SAI CLINIC專科醫生進行遠程視頻諮詢，由專業醫療翻譯全程陪同。醫生將詳細說明線雕提升、雙眼皮、玻尿酸注射等療程的適用性、療程時程、恢復期、費用明細等，幫助您做出赴日醫美的決定。', 'zh-CN': '与SAI CLINIC专科医生进行远程视频咨询，由专业医疗翻译全程陪同。医生将详细说明线雕提升、双眼皮、玻尿酸注射等疗程的适用性、疗程时程、恢复期、费用明细等，帮助您做出赴日医美的决定。', en: 'Remote video consultation with SAI CLINIC cosmetic surgeon, with professional medical interpreter. Doctor will explain suitability for thread lift, double eyelid surgery, filler injections, treatment schedule, downtime, and cost details.' } as Record<Language, string>,
  taxIncluded: { ja: '日円（税込）', 'zh-TW': '日円（税込）', 'zh-CN': '日元（含税）', en: 'JPY (tax incl.)' } as Record<Language, string>,
  videoConsultation: { ja: 'ビデオ相談', 'zh-TW': '視頻諮詢', 'zh-CN': '视频咨询', en: 'Video Consultation' } as Record<Language, string>,
  backToMain: { ja: 'SAI CLINICに戻る', 'zh-TW': '返回SAI CLINIC', 'zh-CN': '返回SAI CLINIC', en: 'Back to SAI CLINIC' } as Record<Language, string>,
  featuresTitle: { ja: 'サービス内容', 'zh-TW': '服務包含', 'zh-CN': '服务包含', en: 'Service Includes' } as Record<Language, string>,
  feature1: { ja: 'SAI CLINIC専門医とのビデオ相談（約30-60分）', 'zh-TW': 'SAI CLINIC專科醫生視頻諮詢（約30-60分鐘）', 'zh-CN': 'SAI CLINIC专科医生视频咨询（约30-60分钟）', en: 'Video consultation with SAI CLINIC cosmetic surgeon (30-60 mins)' } as Record<Language, string>,
  ...
  feature3: { ja: '美容整形施術の適応性評価', 'zh-TW': '醫美整形療程適用性評估', 'zh-CN': '医美整形疗程适用性评估', en: 'Cosmetic procedure suitability assessment' } as Record<Language, string>,
  feature4: { ja: '個別施術計画とスケジュール説明', 'zh-TW': '個人療程計劃與時程說明', 'zh-CN': '个人疗程计划与时程说明', en: 'Personalized treatment plan and schedule' } as Record<Language, string>,
  feature5: { ja: 'ダウンタイムと費用明細見積', 'zh-TW': '恢復期說明與費用明細報價', 'zh-CN': '恢复期说明与费用明细报价', en: 'Downtime information and detailed cost quotation' } as Record<Language, string>,
  feature6: { ja: '相談後の書面総括報告', 'zh-TW': '諮詢後書面總結報告', 'zh-CN': '咨询后书面总结报告', en: 'Written summary report after consultation' } as Record<Language, string>,
  ...
  prereq2: { ja: '希望施術に関する情報・写真提出済み', 'zh-TW': '已提交希望療程相關資料及照片', 'zh-CN': '已提交希望疗程相关资料及照片', en: 'Submitted information and photos regarding desired procedures' } as Record<Language, string>,
  prereq3: { ja: 'SAI CLINICの初期評価完了', 'zh-TW': 'SAI CLINIC已完成初步評估', 'zh-CN': 'SAI CLINIC已完成初步评估', en: 'SAI CLINIC completed initial assessment' } as Record<Language, string>,
  ...
  cancerType: { ja: 'ご希望の施術', 'zh-TW': '希望的療程', 'zh-CN': '希望的疗程', en: 'Desired Procedure' } as Record<Language, string>,
  cancerTypePlaceholder: { ja: '例：糸リフト、二重整形、ヒアルロン酸等', 'zh-TW': '例如：線雕提升、雙眼皮、玻尿酸等', 'zh-CN': '例如：线雕提升、双眼皮、玻尿酸等', en: 'e.g., Thread lift, double eyelid, hyaluronic acid, etc.' } as Record<Language, string>,
  previousConsultation: { ja: '初期相談・美容施術歴の状況', 'zh-TW': '前期諮詢及醫美經歷', 'zh-CN': '前期咨询及医美经历', en: 'Previous Consultation & Cosmetic History' } as Record<Language, string>,
  previousConsultationPlaceholder: { ja: '初期相談の完了状況、過去の美容施術歴等をご記入ください', 'zh-TW': '請說明是否已完成前期諮詢、過往醫美經歷等', 'zh-CN': '请说明是否已完成前期咨询、过往医美经历等', en: 'Please describe completion status of initial consultation and any previous cosmetic procedures' } as Record<Language, string>,
  questionsToAsk: { ja: '相談時にお聞きになりたいこと', 'zh-TW': '希望諮詢的問題', 'zh-CN': '希望咨询的问题', en: 'Questions for Consultation' } as Record<Language, string>,
  questionsToAskPlaceholder: { ja: '相談時に医師に質問したい内容をご記入ください（施術方法、ダウンタイム、費用等）', 'zh-TW': '請列出您希望在諮詢中詢問醫生的問題（療程方式、恢復期、費用等）', 'zh-CN': '请列出您希望在咨询中询问医生的问题（疗程方式、恢复期、费用等）', en: 'Please list questions you want to ask the doctor (procedure details, downtime, costs, etc.)' } as Record<Language, string>,
  preferredTimesTitle: { ja: 'ご希望相談時間', 'zh-TW': '希望諮詢時間', 'zh-CN': '希望咨询时间', en: 'Preferred Consultation Times' } as Record<Language, string>,
  ...
  paymentNotice: { ja: 'お支払い完了後、24時間以内にご連絡し、相談時間を確定いたします', 'zh-TW': '支付完成後，我們將在 24 小時內與您聯繫，確認諮詢時間', 'zh-CN': '支付完成后，我们将在 24 小时内与您联系，确认咨询时间', en: 'We will contact you within 24 hours after payment to confirm consultation time' } as Record<Language, string>,
...
const SERVICE_INFO = {
  id: MEDICAL_PACKAGES['sai-remote-consultation'].slug,
  slug: MEDICAL_PACKAGES['sai-remote-consultation'].slug,
  price: MEDICAL_PACKAGES['sai-remote-consultation'].priceJpy,
};

export default function SaiClinicRemoteConsultationPage() {
  ...
  const backHref = guideSlug ? `/g/${guideSlug}/sai-clinic` : '/sai-clinic';
  ...
  const [patientInfo, setPatientInfo] = useState({ patientName: '', age: '', gender: '', cancerType: '', previousConsultation: '', questions: '' });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      fullNotes += `${t('cancerType')}: ${patientInfo.cancerType || '未提供'}\n`;
      ...
      const response = await fetch('/api/create-checkout-session', {
        ...
        body: JSON.stringify({
          ...
          preferredTime: null,
          notes: fullNotes,
          provider: providerKey || 'sai-clinic',
          locale: currentLang,
          ...(guideSlug ? { guideSlug } : {}),
        }),
      });
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-blue-200 text-sm">{t('videoConsultation')}</span>
              </div>
            ...
            <div className="bg-blue-50 rounded-2xl p-6 border border-blue-100 sticky top-8">
              ...
              <div className="space-y-2.5 text-sm text-gray-700">
                {[t('feature1'), t('feature2'), t('feature3'), t('feature4'), t('feature5'), t('feature6')].map((feature, idx) => (
                  <div key={idx} className="flex gap-2"><CheckCircle size={16} className="shrink-0 mt-0.5 text-blue-500" /><span>{feature}</span></div>
                ))}
              </div>
              <div className="mt-6 pt-6 border-t border-blue-200">
                <h4 className="font-bold text-gray-900 mb-3 flex items-center gap-2"><FileText size={16} className="text-blue-600" />{t('prerequisitesTitle')}</h4>
                <ul className="space-y-2 text-sm text-gray-600">
                  {[t('prereq1'), t('prereq2'), t('prereq3')].map((prereq, idx) => (
                    <li key={idx} className="flex gap-2"><span className="text-blue-400">•</span><span>{prereq}</span></li>
                  ))}
                </ul>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-2xl border border-gray-200 p-8">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6">
                <p className="text-sm text-blue-800"><strong>{t('contractNotice').split('：')[0]}：</strong>{t('contractNotice').split('：')[1]}</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('cancerType')}</label>
                      <input type="text" value={patientInfo.cancerType} onChange={(e) => setPatientInfo({ ...patientInfo, cancerType: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder={t('cancerTypePlaceholder')} />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('previousConsultation')}</label>
                      <textarea value={patientInfo.previousConsultation} onChange={(e) => setPatientInfo({ ...patientInfo, previousConsultation: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent" rows={2} placeholder={t('previousConsultationPlaceholder')} />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('questionsToAsk')}</label>
                      <textarea value={patientInfo.questions} onChange={(e) => setPatientInfo({ ...patientInfo, questions: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent" rows={3} placeholder={t('questionsToAskPlaceholder')} />
                    </div>
                  </div>
                </div>

                <div>
                  <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><Clock size={18} className="text-blue-600" />{t('preferredTimesTitle')}</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <input type="text" value={preferredTimes.time1} onChange={(e) => setPreferredTimes({ ...preferredTimes, time1: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder={t('preferredTimePlaceholder1')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('preferredTime2')}</label>
                      <input type="text" value={preferredTimes.time2} onChange={(e) => setPreferredTimes({ ...preferredTimes, time2: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder={t('preferredTimePlaceholder2')} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{t('preferredTime3')}</label>
                      <input type="text" value={preferredTimes.time3} onChange={(e) => setPreferredTimes({ ...preferredTimes, time3: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder={t('preferredTimePlaceholder3')} />
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent" rows={3} placeholder={t('additionalNotesPlaceholder')} />
                </div>
                ...
                <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
                  ...
                  <button type="submit" disabled={processing} className="w-full py-4 bg-gradient-to-r from-blue-600 to-blue-800 text-white font-bold rounded-xl hover:from-blue-700 hover:to-blue-900 transition shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                ...
                <div className="flex flex-wrap justify-center gap-6 text-xs text-gray-500">
                  ...
                  <div className="flex items-center gap-1"><Clock size={14} className="text-blue-500" /><span>{t('confirmTime24h')}</span></div>
                </div>
              </form>
            </div>
          </div>
