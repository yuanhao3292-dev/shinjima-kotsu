...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 跳到固定地址而不是留在本页用局部状态展示输入框：
      // 用户要切到邮箱客户端抄验证码，回来时本页 React 状态早就没了。
      window.sessionStorage.setItem('pending_signup_email', email.trim().toLowerCase());
            ...
            <div className="max-w-lg">
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <span className="text-white/85">{t('benefit1', lang)}</span>
                </div>
                <div className="flex items-center gap-3 text-sm">
                  <CheckCircle className="w-4 h-4 text-white/90" />
                  <span className="text-white/85">{t('benefit2', lang)}</span>
                </div>
                <div className="flex items-center gap-3 text-sm">
                  <CheckCircle className="w-4 h-4 text-white/90" />
                  <span className="text-white/85">{t('benefit3', lang)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="w-full max-w-md">
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full brand-gradient-solid hover:opacity-90 disabled:bg-neutral-300 text-white font-medium py-3 px-6 text-sm tracking-wider transition-colors flex items-center justify-center gap-2 mt-2"
              >
                {loading ? (
                  <>
