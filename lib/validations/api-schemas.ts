...
export const ModuleDataSchema = z.object({
  category: z.string().min(1, '模块分类不能为空').max(50, '模块分类最多50个字符'),
  ...
  thumbnailUrl: z.string().url('缩略图URL无效').nullable().optional(),
  commissionRateA: z.number().min(0, 'Growth佣金比例不能为负').max(100, 'Growth佣金比例不能超过100').default(10),
  commissionRateB: z.number().min(0, 'Partner佣金比例不能为负').max(100, 'Partner佣金比例不能超过100').default(20),
