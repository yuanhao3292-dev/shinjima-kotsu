...
|------|---------------|------|
| 主色 藍 | `brand-900` (#12384c) | 深色背景、标题文字、hover 状态 |
| 金茶点睛 | `gold-400` (#c9a55f) | **仅关键 CTA / 重点强调**，不作全站装饰铺陈 |
| 正文 | `neutral-700` | 段落文字 |
| 次要文字 | `neutral-500` / `neutral-400` | 辅助说明、日期 |
| 浅背景 | `neutral-50` | 交替 section 背景 |
| 边框 | `neutral-200` | 分割线、卡片边框 |
| 板块识别色 | `medical-*` 青 / `biz-*` 蓝 / `gold-*` 金茶 | 医疗 / 商务 / 高尔夫，**用颜色区分业务线** |

**禁止使用：** `gray-*`（用 `neutral-*` 替代）、`slate-*`（用 `brand-*` 替代）、`blue-600` 链接色（用 `brand-700` 替代）

#### ⚠️ 配色方针（2026-08 定稿：晴空渐变，勿回退）

原色板为 `brand-900 #102a43`（冷调深蓝）+ `gold-400 #d4af37`（金属金）。业务方反馈该组合「压抑、偏硬、太AI化」。参考 JTB 官网后定稿为**柔和阳光型渐变主色**：

1. **主色 晴空蓝** — `brand-500 #4e9ac9`。相较原深蓝，饱和度下调约四成、明度抬高，取 JTB 首屏淡蓝天空的通透感。深色端 `brand-900 #22455a` 仅用于标题文字，不再作大面积背景。
2. **金色已全站移除** — `gold-*` 已重命名为 `accent-*`（457 处 / 58 文件），色值换为**与原金色逐档等明度**的晴空浅阶（如 gold-400 #c9a55f → accent-400 #75b1d7，明度 0.4015→0.4013）。等明度是刻意设计：旧用法的对比关系原样保留，不会出现浅底配浅字。**改 accent 明度前必须重算对比度。**
   注意：`goldPartner`/`goldBar` 等业务术语与变量名未改，勿一并替换。
3. **渐变只用于「该发光的地方」** — 色带、按钮、标签下划线、统计数字、业务分区块；大面积仍是白底。清爽感来自留白，不是色块。
   - 装饰色带：`.brand-gradient`（#4e9ac9 → #bfe3f3）
   - 按钮/承载白字：`.brand-gradient-solid`（#336a8d → #4e9ac9）
   - 渐变文字：`.brand-gradient-text`
   - ⚠️ **按钮渐变勿再调浅**：现值白字对比度深端 5.85:1、亮端 3.09:1，已是柔和度与可读性的临界点。
4. **版块以白底为主** — 首页原有 5 个满屏深色压图版块（占 72% 版面）已改为白底 + 右侧照片透出。**新增版块默认白底**，深色仅用于 hero。
5. **正文柔和近黑** — `#333`（与 JTB #323232、日本数字厅 Solid Gray 800 一致），不用纯黑。

依据：JTB 实测配色、英国 NHS 设计系统 `#005EB8`、日本数字厅 design-tokens。
