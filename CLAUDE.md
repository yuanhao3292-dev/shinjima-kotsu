...
#### ⚠️ 配色方针（2026-08 调整，勿回退）

原色板为 `brand-900 #102a43`（冷调企业蓝灰）+ `gold-400 #d4af37`（高亮金属金）。这一组合是通用「AI 生成高端模板」的默认输出，观感生硬、缺乏人味。参考 JTB 等日本本土站点的做法后调整为：

1. **藍 (Ai)** — `brand-*` 改用日本传统藍色系（偏青、更暖），与 `medical-*` 青自然衔接。
2. **金茶** — `gold-*` 由金属金降为柔和金茶/真鍮，**降级为点睛色**，不再全站描边铺陈。
3. **提亮** — Hero/版块照片的深色渐变遮罩整体降档（原 `from-brand-900/95` 一类降至 `/70` 左右），让照片保持自然明度。**新增版块遮罩不要超过 `/70`。**
4. **柔和对比** — 正文近黑取 `#2D2D2D` 级（已在 body），不用纯黑；避免把黑白对比拉满。
5. **颜色承担信息** — 强调色优先用于区分业务线/状态，而非纯装饰。

弹窗背板（`bg-black/50` 等）**不在提亮范围内**，需保持压暗以聚焦。
