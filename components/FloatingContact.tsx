...
const contactTranslations = {
  ...
  lineTitle: { ja: 'LINE相談', 'zh-TW': 'LINE 諮詢', 'zh-CN': 'LINE 咨询', en: 'LINE Chat' },
  lineAddId: { ja: 'LINE IDを追加：', 'zh-TW': '請添加 LINE ID：', 'zh-CN': '请添加 LINE ID：', en: 'Add LINE ID:' },
  lineAfterAdd: { ja: '追加後にご用件をお伝えください', 'zh-TW': '添加後請說明來意', 'zh-CN': '添加后请说明来意', en: 'Please state your inquiry after adding' },
  lineOnline: { ja: 'LINE オンライン', 'zh-TW': 'LINE 客服在線', 'zh-CN': 'LINE 客服在线', en: 'LINE Online' },
  lineOpenProfile: { ja: 'LINEで開く', 'zh-TW': '用 LINE 開啟', 'zh-CN': '用 LINE 打开', en: 'Open in LINE' },
  legalProvider: {
    ...
    <>
        ...
        >
          ...
          >
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-bold text-lg text-neutral-900">{ct('wechatTitle')}</h3>
              <button
                onClick={() => setShowWechatQR(false)}
            ...
            <div className="bg-neutral-50 rounded-xl p-4 flex flex-col items-center justify-center">
              {/* 白标模式显示微信号，官方模式显示二维码 */}
              {displayWechat ? (
                <div className="text-center py-8">
                  <p className="text-neutral-500 text-sm mb-3">{ct('wechatAddId')}</p>
                  <p className="text-2xl font-bold text-neutral-800 select-all">{displayWechat}</p>
                </div>
              ) : (
                <Image
            ...
            <div className="mt-4 text-center text-xs px-3 py-2 rounded-lg bg-brand-50 text-brand-700">
              {ct('wechatOnline')}
            </div>
          </div>
        </div>
      ...
      {showLineId && (
        ...
        >
          ...
          >
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-bold text-lg text-neutral-900">{ct('lineTitle')}</h3>
              <button
                onClick={() => setShowLineId(false)}
                className="text-neutral-400 hover:text-neutral-600 p-1"
              >
            ...
            <div className="bg-neutral-50 rounded-xl p-4 flex flex-col items-center justify-center">
              <div className="text-center py-8">
                <p className="text-neutral-500 text-sm mb-3">{ct('lineAddId')}</p>
                <p className="text-2xl font-bold text-neutral-800 select-all">{displayLine}</p>
              </div>
            </div>
            ...
            <a
              ...
              className="mt-4 w-full flex items-center justify-center gap-2 brand-gradient-solid hover:opacity-90 text-white py-3 rounded-xl font-medium transition-all duration-200"
            >
      ...
      <div className="fixed bottom-6 right-6 z-50">
        {/* 展开的联系方式菜单 */}
        {isOpen && (
          <div className="absolute bottom-16 right-0 bg-white rounded-2xl shadow-2xl border border-neutral-100 p-4 w-72 animate-fade-in-up">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-bold text-neutral-900">{ct('title')}</h3>
              <button
            ...
            <div className="space-y-3">
              {/* LINE 咨询 - 白标模式弹窗显示 ID，官方模式跳转 LINE 页面 */}
              {displayLine ? (
                <button
                  onClick={() => setShowLineId(true)}
                  className="w-full flex items-center gap-3 brand-gradient-solid hover:opacity-90 text-white px-4 py-3 rounded-xl transition-all duration-200 shadow-lg shadow-brand-200"
                >
              ...
              <button
                ...
                className="w-full flex items-center gap-3 brand-gradient-solid hover:opacity-90 text-white px-4 py-3 rounded-xl transition-all duration-200"
              >
                <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
        ...
        <button
          ...
          className={`
            ...
            ${isOpen
              ...
              : 'brand-gradient-solid text-white animate-pulse'
            }
          `}
        ...
        {!isOpen && (
          <div className="absolute bottom-16 right-0 bg-white rounded-lg shadow-lg px-4 py-2 whitespace-nowrap animate-bounce">
            <div className="text-sm font-medium text-neutral-800">{ct('bubble')}</div>
